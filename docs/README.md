# Documentation

This directory contains the project documentation for PPTKit.

## Sections

- [Getting Started](getting-started/README.md)
- [Concepts](concepts/README.md)
- [Guides](guides/README.md)
- [API](api/README.md)
- [Plugins](plugins/README.md)
- [Renderer](renderer/README.md)
- [Architecture](architecture/README.md)
- [Migration](migration/README.md)
- [FAQ](faq/README.md)
- [Best Practices](best-practices/README.md)

## Documentation Model

PPTKit documentation is organized into three layers:

1. User docs for installation, quick start, and practical usage.
2. Architecture docs for contributors who need to understand the system design.
3. Decision records such as RFCs for major technical changes.

## Suggested Reading Order

1. [Getting Started](getting-started/README.md)
2. [Architecture Overview](architecture/overview.md)
3. [Design Principles](architecture/design-principles.md)
4. [Document Models](architecture/document-models.md)
5. [Editable vs Visual](architecture/editable-vs-visual.md)
6. [Package Boundaries](architecture/package-boundaries.md)
7. [Rendering Pipeline](architecture/rendering-pipeline.md)
